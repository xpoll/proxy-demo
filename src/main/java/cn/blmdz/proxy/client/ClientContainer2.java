...
import cn.blmdz.proxy.client.handler.FaceProxyChannelHandler2;
import cn.blmdz.proxy.client.handler.FaceServerChannelHandler2;
...
public class ClientContainer2 implements Container {
        ...
        ClientConstant.bootstrapProxy.connect(ClientConstant.SERVER_HOST, ClientConstant.SERVER_PORT).addListener(new ChannelFutureListener() {
            ...
            public void operationComplete(ChannelFuture future) throws Exception {
                if (future.isSuccess()) {
                    ...
                    future.channel().writeAndFlush(Message2.build(MessageType2.AUTH, JSON.toJSONString(ClientConstant.serverParam)));
                } else {
...
//        serverParam = new ProxyRequestServerParam(APPID, CLIENT_PORT);
        ContainerHelper.start(Arrays.asList(new Container[] { new ClientContainer2() }));
    }
}
